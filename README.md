...
3) - 1.2_Sim Est EGIG.SCD_SIR.R - Parameter estimation for EGIG.SCD model
...
2)
   - ggplot2 - elegant and versatile plotting system
   - pracma - practical numerical math functions
   - gridExtra - arranges multiple plots on a single page/grid
     
4) optimx - advanced numerical optimisation with multiple methods and diagnostics
5) - ggplot2,
   - pracma,
   - gridExtra - refer to 2)
   - MASS - provides functions for applied statistics
   - gsl - provides special mathematical functions like gamma, Bessel, and other advanced

### Instructions
Set working directory
1) To combine multiple estimation output files into a single dataset
2) - To generate the corresponding plots
   - To compute empirical density, distribution, survival, and hazard functions using Gamma kernel estimation
...
5) DM test.R - Compares forecast accuracy of multiple models using the DM Test.
6) MCS.R - Evaluates and compares forecast performance using loss functions and the MCS procedure
7) TaR Plot_EGIG.TSCD.R - visualises TaR duration forecasts alongside diurnally adjusted durations

### Software Requirements
R version 2025.09.0
Required packages:
1) optimx - advanced numerical optimisation with multiple methods and diagnostics
2) - GAS - used for backtesting Value-at-Risk (VaR)
   - rmutil - used for the generalised Gamma quantile function
...
2)  -  To compute TaR forecasts, violation statistics, and perform risk backtests
    -  To compute GG distribution quantiles for TaR forecasts
    -  To perform statistical backtests of TaR forecasts (Kupiec, Dynamic Quantile, Christoffersen)
