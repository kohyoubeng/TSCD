###############################################################################
# REPLICATION INSTRUCTIONS — GG.SCD Model Estimation
#
# This script estimates the GG.SCD duration model using a particle filter
...
a.int = 0.3
p.int = 5.5
initialTheta = c(miu.int, be.int, sig.int, a.int, p.int)
...
# ===================== Optimization =====================
OP = optimx::optimx(par=initialTheta, fn=function(par) LL(par, x.obs),
                    gr=NULL, hess=NULL,
                    hessian=FALSE,
                    lower=c(-Inf, -0.99, 0, 0, 0),
                    upper=c(Inf, 0.99, Inf, Inf, Inf),
                    method=c("L-BFGS-B"), 
