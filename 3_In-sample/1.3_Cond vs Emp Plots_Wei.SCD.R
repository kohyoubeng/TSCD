###############################################################################
# Conditional vs Empirical PDF/CDF/Survival/Hazard (Wei SCD Model)
#
# This script compares the conditional Wei–SCD model functions with the
...
# • Update the Wei shape parameter value **a** to the TSLA estimate
#
# Notes:
# • MUST ensure the input CSV contains columns:
#     x.use  (diurnally adjusted durations)
#     Wei    (fitted conditional mean from Wei–SCD model)
...
compute_pdf = function(eps, phi, a, b) {
  (a / b^a) * eps^(a - 1) / phi * exp(-(eps / b)^a)
}

compute_cdf = function(eps, a, b) {
  1 - exp(-(eps / b)^a)
}
...
cond_pdf = compute_pdf(eps_gridCond, median_phi, a, b)
cond_cdf = compute_cdf(eps_gridCond, a, b)
...
###############################################################################
# Step 5: Interpolate conditional results to empirical grid
...
cond_survival_interp = approx(x_gridCond, cond_survival, xout = eps_grid, rule = 2)$y
...
write.csv(result_df, "AAPL Cond vs Emp_Wei.SCD.csv", row.names = FALSE)
...
line_type_settings = scale_linetype_manual(
  values = c("Empirical" = "solid", "SCD_Wei model" = "dotted"),
  labels = c("Empirical", expression("SCD"[Wei] * " model"))
)

# PDF plot
pdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  geom_line(aes(y = empirical_pdf, linetype = "Empirical"), color="black") +
  geom_line(aes(y = conditional_pdf, linetype = "SCD_Wei model"), color="blue") +
...
cdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_cdf, linetype = "SCD_Wei model"), color= "blue") +
...
survival_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_survival, linetype = "SCD_Wei model"), color = "blue") +
...
hazard_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_hazard, linetype = "SCD_Wei model"), color="blue") +
...
combined_plot = grid.arrange(pdf_plot, cdf_plot, survival_plot, hazard_plot, 
                             ncol = 1)
ggsave("AAPL combined Cond plot_Wei.SCD.png", plot = combined_plot, width = 16, 
       height = 12, dpi = 300)
...
ggsave("AAPL Hazard_Wei.SCD.png", plot = hazard_plot_no_title, width = 16, 
       height = 3, dpi = 300)

##############################################################################
# Open a PNG device
png("AAPL Adj Dur Histogram.png", width = 1200, height = 800, res = 150)

# Your histogram code
hist(
  x,
  breaks = 100,
  main = "AAPL Adj Dur Histogram",
  xlab = "Adjusted Duration",
  col = "black",
  border = "white"
)
