###############################################################################
# Wei.SCD Model Estimation
#
# This script estimates the Wei.SCD duration model using a particle filter
...
sig.int = 0.3
a.int = 0.5
initialTheta = c(miu.int, be.int, sig.int, a.int)
...
LL = function(theta, x) {  
  ...
  for(i in 1:n) {
    ...
    # ================= Calculate weight =================
    g = gamma(1 + 1/a)
    un.weight[i, ] = a * g^a * x[i]^(a - 1) / (exp(psi[i, ]))^a * 
      exp(-((x[i] * g) / exp(psi[i, ]))^a)
