###############################################################################
# Conditional vs Empirical PDF/CDF/Survival/Hazard (GG SCD Model)
#
# This script compares the conditional GG–SCD model functions with the
...
# • Update the GG shape parameter value **a** and **p** to the TSLA estimate
#
# Notes:
# • MUST ensure the input CSV contains columns:
#     x.use  (diurnally adjusted durations)
#     GG    (fitted conditional mean from GG–SCD model)
...
library(ggplot2)
library(gridExtra)
library(MASS)      # for bandwidth estimation
library(zipfR)     # for incomplete gamma function

###############################################################################
# Step 1: Load data and parameters

par.read = c(0.325214641666, 5.546737296414)
a = par.read[1]  # shape parameter
p = par.read[2]  # shape parameter
b = gamma(p) / gamma(p + 1/a)
...
compute_pdf = function(eps, phi, a, p, b) {
  a / (b^(a * p) * gamma(p)) * (eps)^(a * p - 1) / phi * exp(- (eps / b)^a)
}

compute_cdf = function(eps, la, p, b) {
  x_LIGF = eps * gamma(p + 1/a) / gamma(p)
  LIGF = zipfR::Igamma(p, x_LIGF^a, lower = TRUE, log = FALSE)
  LIGF / gamma(p)
}
...
cond_pdf = compute_pdf(eps_gridCond, median_phi, a, p, b)
cond_cdf = compute_cdf(eps_gridCond, a, p, b)
...
write.csv(result_df, "AAPL Cond vs Emp_GG.SCD.csv", row.names = FALSE)
...
line_type_settings = scale_linetype_manual(
  values = c("Empirical" = "solid", "SCD_GG model" = "dotted"),
  labels = c("Empirical", expression("SCD"[GG] * " model"))
)

# PDF plot
pdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  geom_line(aes(y = empirical_pdf, linetype = "Empirical"), color="black") +
  geom_line(aes(y = conditional_pdf, linetype = "SCD_GG model"), color="blue") +
...
cdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_cdf, linetype = "SCD_GG model"), color= "blue") +
...
survival_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_survival, linetype = "SCD_GG model"), color = "blue") +
...
hazard_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_hazard, linetype = "SCD_GG model"), color="blue") +
...
combined_plot = grid.arrange(pdf_plot, cdf_plot, survival_plot, hazard_plot, 
                             ncol = 1)
ggsave("AAPL combined Cond plot_GG.SCD.png", plot = combined_plot, width = 16, 
       height = 12, dpi = 300)
...
ggsave("AAPL Hazard_GG.SCD.png", plot = hazard_plot_no_title, width = 16, 
       height = 3, dpi = 300)
