###############################################################################
# PSI ESTIMATION — Compute latent states for Wei.SCD model
#
...
# • Output files are named like "AAPL psi & exp(psi)_Wei.SCD.csv" 
#   or "TSLA psi & exp(psi)_Wei.SCD.csv"
###############################################################################
...
x.read = read.csv("Adj dur AAPL In-sample (t)_29 Sep-10 Oct 2025.csv", 
                  header=TRUE)
par.read = read.csv("AAPL Est Wei.SCD_500.csv", header=TRUE)
...
psi = my_func(x.obs, miu, be, sig, a)
phi = exp(psi)

# Combine ψ_t and exp(ψ_t)
out = cbind(psi, phi)

write.csv(out, "AAPL psi & exp(psi)_Wei.SCD.csv", row.names=F)
