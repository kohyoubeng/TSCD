###############################################################################
# Conditional vs Empirical PDF/CDF/Survival/Hazard (GB2 SCD Model)
#
# This script compares the conditional GB2–SCD model functions with the
...
# • Update the GB2 shape parameter value **a**, **p** and **q** to the TSLA estimate
#
# Notes:
# • MUST ensure the input CSV contains columns:
#     x.use  (diurnally adjusted durations)
#     GB2    (fitted conditional mean from GB2–SCD model)
...
par.read = c(0.635400933526, 2.760359422844, 24.98463594218)
a = par.read[1]
p = par.read[2]
q = par.read[3]
b = beta(p, q) / beta(p + 1/a, q - 1/a)
...
compute_pdf = function(eps, phi, a, q, b) {
  a * (eps)^(a*p - 1) / 
    (b^(a*p) * beta(p, q) * (1 + ((eps)/b)^a)^(p+q)) / phi
}

compute_cdf = function(eps, a, q, b) {
  b.1 = beta(a=p, b=q)
  b.2 = beta(a=p+1/a, b=q-1/a)
  
  # Calculate z for the CDF
  z = (eps * b.2 / b.1)^a / (1 + (eps * b.2 / b.1)^a)
  
  cdf_value = zipfR::Ibeta(z, p, q, lower=TRUE, log=FALSE) / beta(p, q)
...
write.csv(result_df, "AAPL Cond vs Emp_GB2.SCD.csv", row.names = FALSE)
...
line_type_settings = scale_linetype_manual(
  values = c("Empirical" = "solid", "SCD_GB2 model" = "dotted"),
  labels = c("Empirical", expression("SCD"[GB2] * " model"))
)

# PDF plot
pdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  geom_line(aes(y = empirical_pdf, linetype = "Empirical"), color="black") +
  geom_line(aes(y = conditional_pdf, linetype = "SCD_GB2 model"), color="blue") +
...
cdf_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_cdf, linetype = "SCD_GB2 model"), color="blue") +
...
survival_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_survival, linetype = "SCD_GB2 model"), color="blue") +
...
hazard_plot = ggplot(filtered_data, aes(x = eps_i)) +
  ...
  geom_line(aes(y = conditional_hazard, linetype = "SCD_GB2 model"), color="blue") +
...
combined_plot = grid.arrange(pdf_plot, cdf_plot, survival_plot, hazard_plot, 
                             ncol = 1)
ggsave("AAPL combined Cond plot_GB2.SCD.png", plot = combined_plot, width = 16, 
       height = 12, dpi = 300)
...
ggsave("AAPL Hazard_GB2.SCD.png", plot = hazard_plot_no_title, width = 16, 
       height = 3, dpi = 300)
