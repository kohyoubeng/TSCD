# =============================================================================
# One-Step-Ahead Latent State Forecasting and Loss Func – Wei.SCD
#
# NOTE:
...
# • Replace "AAPL Roll pars_Wei.SCD.csv" with "TSLA Roll pars_Wei.SCD.csv"
...
par.read = read.csv("AAPL Roll pars_Wei.SCD.csv", header=TRUE)
...
my_func = function(x.obs, miu, be, sig, a){
...
for(j in 1:nf) {
  ...
  res = my_func(x.obs, miu, be, sig, a)
...
write.csv(fpsi, "AAPL fpsi_Wei.SCD.csv", row.names=TRUE)
write.csv(forc1, "AAPL forc_Wei.SCD.csv", row.names=FALSE)
...
# Reload forecasts
forecast.df = read.csv("AAPL forc_Wei.SCD.csv", header=TRUE)
...
# Save to CSV
write.csv(Out_combined, "AAPL Forc Error_Wei.SCD.csv", row.names = FALSE)
