###############################################################################
# TaR Forecasting and Backtesting — EGIG SCD Model
#
...
#     "AAPL Roll pars_EGIG.SCD.csv" → "TSLA Roll pars_EGIG.SCD.csv"
...
fphi = data$EGIG  #fphi = exp(fpsi)

#read forecasted parameters
fpar.read = read.csv("AAPL Roll pars_EGIG.SCD.csv",header=TRUE)
...
myfunc = function(y2){
  y2^(lam/del - 1) * exp(-y2 - w^2/4 * y2^(-1))
}
###############################################################################
...
for(h1 in 1:length(u)){
  for(i in 1:n.f){
    ...
    tar = 2*u[h1]*(w/2)^(lam/del) * b1
    z = findprob(myfunc, interval=c(0,20), target=tar)
    
    fTaR[i] = fphi[i] * (2*z/w)^(1/del) * b1/b2
...
write.csv(out.forc, "AAPL ObsDur, fphi & TaR Forecast_EGIG.SCD.csv")
...
write.csv(out.RVR, "AAPL RVR Forecast_EGIG.SCD.csv")
...
write.csv(out.fKup, "AAPL Kupiec Forecast_EGIG.SCD.csv")
write.csv(out.fDQ, "AAPL DQ Forecast_EGIG.SCD.csv")
write.csv(out.fCC, "AAPL CC Forecast_EGIG.SCD.csv")
