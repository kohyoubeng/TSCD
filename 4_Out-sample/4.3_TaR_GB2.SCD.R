###############################################################################
# TaR Forecasting and Backtesting — GB2 SCD Model
#
...
#     "AAPL Roll pars_GB2.SCD.csv" → "TSLA Roll pars_GB2.SCD.csv"
...
fphi = data$GB2  #fphi = exp(fpsi)

#read forecasted parameters
fpar.read = read.csv("AAPL Roll pars_GB2.SCD.csv",header=TRUE)
...
for(h1 in 1:length(u)){
  for(i in 1:n.f){
    ...
    z = qbeta(u[h1], p, q, ncp=0, lower.tail=TRUE, log.p=FALSE)
    
    fTaR[i] = fphi[i] * b * (z/(1-z))^(1/a)
...
write.csv(out.forc, "AAPL ObsDur, fphi & TaR Forecast_GB2.SCD.csv")
...
write.csv(out.RVR, "AAPL RVR Forecast_GB2.SCD.csv")
...
write.csv(out.fKup, "AAPL Kupiec Forecast_GB2.SCD.csv")
write.csv(out.fDQ, "AAPL DQ Forecast_GB2.SCD.csv")
write.csv(out.fCC, "AAPL CC Forecast_GB2.SCD.csv")
