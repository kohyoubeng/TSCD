###############################################################################
# TaR Forecasting and Backtesting — Wei SCD Model
#
...
#     "AAPL Roll pars_Wei.SCD.csv" → "TSLA Roll pars_Wei.SCD.csv"
...
fphi = data$Wei  #fphi = exp(fpsi)

#read forecasted parameters
fpar.read = read.csv("AAPL Roll pars_Wei.SCD.csv",header=TRUE)
...
for(h1 in 1:length(u)){
  for(i in 1:n.f){
    ...
    a = fpar$a[i]
    b = 1 / gamma(1 + 1/a)
    fTaR[i] = fphi[i] * b * (-log(1-u[h1]))^(1/a)
    
    #use R build-in func
    # fTaR[i] = fphi[i] * qweibull(p=u[h1], shape=a, scale=b, lower.tail = TRUE, 
    #                              log.p = FALSE)
...
write.csv(out.forc, "AAPL ObsDur, fphi & TaR Forecast_Wei.SCD.csv")
...
write.csv(out.RVR, "AAPL RVR Forecast_Wei.SCD.csv")
...
write.csv(out.fKup, "AAPL Kupiec Forecast_Wei.SCD.csv")
write.csv(out.fDQ, "AAPL DQ Forecast_Wei.SCD.csv")
write.csv(out.fCC, "AAPL CC Forecast_Wei.SCD.csv")
