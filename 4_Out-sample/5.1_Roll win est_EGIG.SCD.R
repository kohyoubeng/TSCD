###############################################################################
# Rolling Window Parameter Estimation — EGIG SCD Model (Particle Filter, SIR)
#
# This script performs rolling-window maximum likelihood estimation of the
# EGIG–SCD model using a SIR particle filter to approximate the likelihood.
#
# The procedure:
...
for(k in 1:n.f){
  ...
  init1 = c(0.6, 0.97, 0.01, 1.1, 0.2, 0.1)
  ...
  #------------------------------------------
  file.name = paste0("AAPL roll pars_EGIG.SCD_win", k, ".csv")
