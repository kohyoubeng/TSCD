###############################################################################
# Rolling Window Parameter Estimation — Burr SCD Model (Particle Filter, SIR)
#
# This script performs rolling-window maximum likelihood estimation of the
# Burr–SCD model using a SIR particle filter to approximate the likelihood.
#
# The procedure:
...
for(k in 1:n.f){
  ...
  q.est = OP$p4
  sig.est = OP$p5
  par.est = c(miu.est, be.est, sig.est, a.est, q.est)
  ...
  #------------------------------------------
  file.name = paste0("AAPL roll pars_Burr.SCD_win", k, ".csv")
