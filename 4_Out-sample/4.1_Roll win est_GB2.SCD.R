###############################################################################
# Rolling Window Parameter Estimation — GB2 SCD Model (Particle Filter, SIR)
#
# This script performs rolling-window maximum likelihood estimation of the
# GB2–SCD model using a SIR particle filter to approximate the likelihood.
#
# The procedure:
...
for(k in 1:n.f){ 
  ...
  init1 = c(0.6, 0.9, 0.6, 2.8, 25, 0.2)
  ...
  q.est = OP$p5
  sig.est = OP$p6
  par.est = c(miu.est, be.est, sig.est, a.est, p.est, q.est)
  ...
  #------------------------------------------
  file.name = paste0("AAPL roll pars_GB2.SCD_win", k, ".csv")
