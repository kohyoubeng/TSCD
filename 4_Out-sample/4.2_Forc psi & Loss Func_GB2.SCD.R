# =============================================================================
# One-Step-Ahead Latent State Forecasting and Loss Func – GB2.SCD
#
# NOTE:
...
# • Replace "AAPL Roll pars_GB2.SCD.csv" with "TSLA Roll pars_GB2.SCD.csv"
...
par.read = read.csv("AAPL Roll pars_GB2.SCD.csv", header=TRUE)
...
for(j in 1:nf) {
  ...
  res = my_func(x.obs, miu, be, sig, a, p, q)
...
write.csv(fpsi, "AAPL fpsi_GB2.SCD.csv", row.names=TRUE)
write.csv(forc1, "AAPL forc_GB2.SCD.csv", row.names=FALSE)
...
# Reload forecasts
forecast.df = read.csv("AAPL forc_GB2.SCD.csv", header=TRUE)
...
# Save to CSV
write.csv(Out_combined, "AAPL Forc Error_GB2.SCD.csv", row.names = FALSE)
