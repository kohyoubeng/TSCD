###############################################################################
# Rolling Window Parameter Estimation — GG SCD Model (Particle Filter, SIR)
#
# This script performs rolling-window maximum likelihood estimation of the
# GG–SCD model using a SIR particle filter to approximate the likelihood.
#
# The procedure:
...
for(k in 1:n.f){ 
  ...
  p.est = OP$p4
  sig.est = OP$p5
  par.est = c(miu.est, be.est, sig.est, a.est, p.est)
  ...
  #------------------------------------------
  file.name = paste0("AAPL roll pars_GG.SCD_win", k, ".csv")
