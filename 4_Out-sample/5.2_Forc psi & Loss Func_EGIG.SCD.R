# =============================================================================
# One-Step-Ahead Latent State Forecasting and Loss Func – EGIG.SCD
#
# NOTE:
...
# • Replace "AAPL Roll pars_EGIG.SCD.csv" with "TSLA Roll pars_EGIG.SCD.csv"
...
par.read = read.csv("AAPL Roll pars_EGIG.SCD.csv", header=TRUE)
...
my_func = function(x.obs, miu, be, sig, lam, del, w){
  ...
  for(i in 1:N){
    ...
    # ================= Calculate weight =================
    #Step 2: Compute unnormalized weight
...
for(j in 1:nf){
  ...
  res = my_func(x.obs, miu, be, sig, lam, del, w)
...
write.csv(fpsi, "AAPL fpsi_EGIG.SCD.csv", row.names=TRUE)
write.csv(forc1, "AAPL forc_EGIG.SCD.csv", row.names=FALSE)
...
# Reload forecasts
forecast.df = read.csv("AAPL forc_EGIG.SCD.csv", header=TRUE)
...
# Save to CSV
write.csv(Out_combined, "AAPL Forc Error_EGIG.SCD.csv", row.names = FALSE)
