###############################################################################
# TaR Forecasting and Backtesting — GG SCD Model
#
...
#     "AAPL Roll pars_GG.SCD.csv" → "TSLA Roll pars_GG.SCD.csv"
...
fphi = data$GG  #fphi = exp(fpsi)

#read forecasted parameters
fpar.read = read.csv("AAPL Roll pars_GG.SCD.csv",header=TRUE)
...
for(h1 in 1:length(u)){
  for(i in 1:n.f){
    ...
    #qggamma(p, s, m, f): p=vector of probabilities, s=p, m=bp, f=a
    fTaR[i] = fphi[i] * rmutil::qggamma(u[h1], s=p, m=(b*p), f=a)
...
write.csv(out.forc, "AAPL ObsDur, fphi & TaR Forecast_GG.SCD.csv")
...
write.csv(out.RVR, "AAPL RVR Forecast_GG.SCD.csv")
...
write.csv(out.fKup, "AAPL Kupiec Forecast_GG.SCD.csv")
write.csv(out.fDQ, "AAPL DQ Forecast_GG.SCD.csv")
write.csv(out.fCC, "AAPL CC Forecast_GG.SCD.csv")
